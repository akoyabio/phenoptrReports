...
In this section, select whether to report results by slide (Slide ID)
or field (Sample Name or Annotation ID).

![](figures/Summarize_by.png)
...
- **Process as whole slide**
  - If this is selected, spatial metrics are computed on data for each slide
as a single data set. If this is not selected, spatial metrics are computed
for each field separately.
  - Only check this if your data consists of contiguous fields for each slide.
  - You must select "Summarize by Slide ID" to use this option.

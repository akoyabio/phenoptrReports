...
process_file = function(path, primary_fluor, percentile=NULL) {
  ...
  if (is.null(percentile)) {
    ...
    percentile = 1-n_bright/prod(dim(primary))
  }
...
The crosstalk table shows the relative signal level of each fluor in
the components where the fluor should not express.

For each image, the brightest `r scales::comma(n_bright)` pixels are found. 
The report computes the mean expression of those pixels in each component.

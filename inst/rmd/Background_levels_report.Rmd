---
title: "Background Levels Report"
date: "`r Sys.Date()`"
...
Background levels report for samples
...
These plots show the relative number of pixels at each intensity for each
...
for (path in comp_files) {
  d = phenoptrReports:::component_ridge_plot(path, bandwidth=NULL)
  print(d$plot)
  cat('\n\n\n')
...
palette = RColorBrewer::brewer.pal(length(levels(all_components$Fluor)), 'Spectral')

purrr::walk2(rev(levels(all_components$Fluor)), rev(palette), 
   function(fluor, fill) {
     print(phenoptrReports:::fluor_ridge_plot(
       all_components %>% filter(Fluor==fluor), fluor, fill, bandwidth=NULL))
})

...
params:
  data_path:
    ...
    value: F:\DavidC\Trillium AC\Analysis\MelanomaResults\Melanoma_170106-01a_Trillium_AllComers_Results.xlsx
  max_slides_per_plot:
    input: integer
...
if ('H-Score' %in% sheet_names) {
  h_score = read_xlsx(data_path, 'H-Score', skip=1)
  ...
  # Reshape for plotting
  tall = h_score %>%
    select(`Slide ID`, `Tissue Category`, `0+`:`3+`) %>%

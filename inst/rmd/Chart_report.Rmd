...
split_slides = function(d) {
  ...
  map(slide_groups, ~d %>% filter(.by_str %in% .x))
}
...
if ('Nearest Neighbors' %in% sheet_names) {
  ...
  iwalk(neighbors, function(d, i) {
    if (length(neighbors) > 1) {
      cat(stringr::str_glue('## Median Distance to Nearest Neighbor ({i} of {length(neighbors)})\n\n'))
    } else {
    ...
    walk(categories, function(category) {
      iwalk(split_counts, function(d, i) {
        ...
        if (length(split_counts) > 1) {
          cat(stringr::str_glue('## Count of "From" cells in {category} with a "To" cell within {radius} microns ({i} of {length(split_counts)})\n\n'))
        } else {
          cat(stringr::str_glue('## Count of "From" cells in {category} with a "To" cell within {radius} microns\n\n'))
        }

...
if ('Cell Counts' %in% sheet_names) {
  ...
  # If there were phenotypes defined, exclude 'Total Cells' from the chart(s)
  if (dplyr::n_distinct(tall$Phenotype) > 1)
    tall = tall %>% filter(Phenotype != 'Total Cells')
...
for (sheet_name in h_score_names) {
  ...
  h_score_title = read_xlsx(workbook_path, sheet_name, range='C1', col_names=FALSE) %>% 
    unlist()

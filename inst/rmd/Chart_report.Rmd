...
if ('Nearest Neighbors' %in% sheet_names) {
  ...
  neighbors = neighbors %>% filter(From != 'Total Cells', To != 'Total Cells')
  ...
  neighbors = split_slides(neighbors)
  iwalk(neighbors, function(d, i) {
    if (length(neighbors) > 1) {
      cat(str_glue('## Median Distance to Nearest Neighbor ({i} of {length(neighbors)})\n\n'))
    } else {
      cat('## Median Distance to Nearest Neighbor\n\n')
    }
    p = ggplot(d, aes(To, From, fill=Median)) +
      ...
      theme(strip.text.x=element_text(face='bold', angle=0),
...
if ('Count Within' %in% sheet_names) {
  ...
  max_counts = counts %>% group_by(Radius, `Tissue Category`) %>% 
    summarize(max=max(`From with`))
  
  # Split to multiple pages
  split_counts = split_slides(counts)
    ...
    walk(categories, function(category) {
      iwalk(split_counts, function(d, i) {
        ...
        if (length(split_counts) > 1) {
          cat(str_glue('## Count of "From" cells in {category} with a "To" cell within {radius} microns ({i} of {length(split_counts)})\n\n'))
        } else {
          cat(str_glue('## Count of "From" cells in {category} with a "To" cell within {radius} microns\n\n'))
        }

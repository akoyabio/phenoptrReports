batch_merge <- function(folders_to_merge) {
  ...
  files_to_catch <-
    expand.grid(folders_to_merge, paste0('Merge', merge_suffixes)) %>%
  ...
  folders_to_merge %>%
    purrr::iwalk( ~ merge_one_sample(
      folders_to_merge_i = .x,
      idx = .y,
...
merge_one_sample <-
  function(folders_to_merge_i, idx, folders_count) {
